...
/* Apple's `hello` script, drawn stroke by stroke on first arrival.
   The two outlines and their timings are lifted from the Framer "Apple Hello Effect" component.
   The drawing itself is CSS — `pathLength="1"` plus a stroke-dashoffset keyframe in globals.css —
   so the overlay draws and clears on its own; this component only decides when the node may
   leave the DOM, and lets an impatient visitor skip ahead. HelloBootstrap has already decided
   whether it plays at all, before paint. */
...
export function HelloIntro() {
  /* Always rendered on the server and on the first client render, so hydration matches. The
     `hello-skip` class on <html> is what hides it instantly when the intro has already played. */
  ...
  useEffect(() => {
    ...
    const clear = () => {
      root.classList.remove('hello-playing')
      setDone(true)
    }
    if (root.classList.contains('hello-skip')) {
      clear()
      return
    }
    const node = ref.current
    if (!node) return
    const skip = () => node.classList.add('is-dismissed')
    const ended = (event: AnimationEvent) => {
      if (event.animationName === 'hello-out') clear()
    }
  ...
  return (
    <div className="hello-intro" ref={ref} aria-hidden="true">
      <svg viewBox="0 0 638 200" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d={STEM} pathLength={1} />
        <path d={SCRIPT} pathLength={1} />
      </svg>
    </div>
  )
}
